    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkEmail) > 0) {
        ...
        if ($data['password'] !== $data['re_password']) {
          ...
          $en_user = str_replace(".", "", $en_user);
          $to       = $data['email'];
          $subject  = "Account Verification - Peninggalan Kebudayaan Bajawa";
                        ...
                        <tr>
                            ...
                            <table role='presentation' border='0' cellpadding='0' cellspacing='0' style='border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; min-width: 100%; width: 100%;' width='100%'>
                                ...
                                <td style='font-family: sans-serif; font-size: 14px; vertical-align: top;' valign='top'>
                                    ...
                                    <p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;'>Terima kasih telah mendaftar di Peninggalan Kebudayaan Bajawa.</p>
  ...
  function forgot_password($conn, $data, $action, $baseURL)
  {
    if ($action == "update") {
      ...
      if (mysqli_num_rows($checkEmail) === 0) {
        ...
        $en_user = str_replace(".", "", $en_user);
        $to       = $data['email'];
        $subject  = "Reset password - Peninggalan Kebudayaan Bajawa";
        $message  = "<!doctype html>
        <html>
          <head>
              <meta name='viewport' content='width=device-width'>
...
if (isset($_SESSION["peninggalan_kebudayaan_bajawa"]["users"])) {
  ...
  function sub_menu($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkSubMenu) > 0) {
        ...
        $_SESSION["peninggalan_kebudayaan_bajawa"]["name_page"] = "' . $data['title'] . '";
  ...
  function jenis_kuliner($conn, $data, $action)
  {
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($view_data) > 0) {
        $message = "Maaf, jenis kebudayaan yang anda masukan sudah ada.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      } else {
  ...
  function tempat_kebudayaan($conn, $data, $action, $deskripsi)
  {
    $path = "../assets/img/wisata/";

    if ($action == "insert") {
      $fileName = basename($_FILES["image_kebudayaan"]["name"]);
      $fileName = str_replace(" ", "-", $fileName);
      $fileName_encrypt = crc32($fileName);
      $ekstensiGambar = explode('.', $fileName);
      $ekstensiGambar = strtolower(end($ekstensiGambar));
      ...
      $allowTypes = array('jpg', 'png', 'jpeg');
      if (in_array($fileType, $allowTypes)) {
        $imageTemp = $_FILES["image_kebudayaan"]["tmp_name"];
        compressImage($imageTemp, $imageUploadPath, 75);
        $image_kebudayaan = $fileName_encrypt . "." . $ekstensiGambar;
      } else {
        $message = "Maaf, hanya file gambar JPG, JPEG, dan PNG yang diizinkan.";
        $message_type = "danger";
        alert($message, $message_type);
      ...
      $sql = "INSERT INTO tempat_kebudayaan(image_kebudayaan,nama_kebudayaan,deskripsi,author) VALUES('$image_kebudayaan','$data[nama_kebudayaan]','$deskripsi','$data[author]')";
    }

    if ($action == "update") {
      if (!empty($_FILES['image_kebudayaan']["name"])) {
        $fileName = basename($_FILES["image_kebudayaan"]["name"]);
        $fileName = str_replace(" ", "-", $fileName);
        $fileName_encrypt = crc32($fileName);
        $ekstensiGambar = explode('.', $fileName);
        ...
        if (in_array($fileType, $allowTypes)) {
          ...
          compressImage($imageTemp, $imageUploadPath, 75);
          $image_kebudayaan = $fileName_encrypt . "." . $ekstensiGambar;
          $remove_image = str_replace($path, "", $data['image_kebudayaanOld']);
          if ($remove_image != "default.svg") {
            unlink($path . $remove_image);
          }
        } else {
        ...
        $image_kebudayaan = $data['image_kebudayaanOld'];
      }
      $sql = "UPDATE tempat_kebudayaan SET image_kebudayaan='$image_kebudayaan', nama_kebudayaan='$data[nama_kebudayaan]', deskripsi='$deskripsi' WHERE id_tempat_kebudayaan='$data[id_tempat_kebudayaan]'";
    }

    if ($action == "delete") {
      unlink($path . $data['image_kebudayaan']);
      $sql = "DELETE FROM tempat_kebudayaan WHERE id_tempat_kebudayaan='$data[id_tempat_kebudayaan]'";
    }
  ...
  function galeri($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if ($data['id_tipe_fitur'] == 1) {
        ...
        $id_fitur = $data['id_tempat_kebudayaan'];
      }
